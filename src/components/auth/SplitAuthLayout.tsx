  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md shadow-sm py-4">
        <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <NavLink to="/login" className="text-sm font-bold text-slate-900 hover:text-brand-600 transition-colors">
              Se connecter
            </NavLink>
          </div>

          {/* Mobile: bouton se connecter */}
          <NavLink to="/login" className="md:hidden text-sm font-bold text-slate-900 hover:text-brand-600">
            Connexion
          </NavLink>
        </div>
      </header>
      ...
      <main className="flex-grow flex items-center justify-center relative w-full pt-20 pb-12 md:px-4 bg-slate-50 overflow-hidden">
        ...
        {/* Split container: formulaire + visual */}
        <div className="w-full max-w-5xl md:bg-white md:rounded-3xl md:shadow-2xl overflow-hidden flex flex-col md:flex-row relative z-10 md:border md:border-slate-100 md:min-h-[600px]">
          ...
          {/* Droite: Visual Panel (caché sur mobile) */}
          <div className="hidden md:flex md:w-1/2 bg-slate-900 relative overflow-hidden items-center justify-center p-12 text-white">
            <div className="absolute inset-0 auth-pattern pointer-events-none" />
            <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-br from-slate-900/90 to-brand-900/40 pointer-events-none" />
            <AuthVisualPanel />
          </div>
        </div>
      </main>
