...
#### 2.9.3.2 Sous-bloc : Heure d'envoi des relances
- [ ] Label : "Heure d'envoi quotidienne"
- [ ] Input type `time` (format HH:MM)
  - Valeur par défaut : `10:00`
  - Plage autorisée : 06:00 - 21:59 (validation frontend + backend)
  - Message d'erreur si heure hors plage : "L'heure d'envoi doit être entre 06:00 et 21:59"
- [ ] Tooltip explicatif : "Les relances automatiques seront générées quotidiennement à cette heure"
- [ ] Sauvegarde immédiate au changement avec toast de confirmation

#### 2.9.3.3 Sous-bloc : Étapes de relance (liste verticale)
...
- [x] Ajouter les champs suivants à la table `organizations` :
  ...
  - `reminderSendTime` (string, optionnel, défaut: "10:00") : Heure d'envoi quotidienne des relances (format "HH:MM", heure locale française naive)
- [x] Anciens champs conservés en optionnel pour rétrocompatibilité

**Note timezone :** Le système assume que tous les utilisateurs sont en France (Europe/Paris). L'heure est stockée en heure locale naive (sans information de timezone). Par exemple, "10:00" signifie 10h du matin heure française, été comme hiver.
...
- [ ] Mutation `organizations.updateReminderSendTime` (validation format HH:MM + plage 06:00-21:59)
